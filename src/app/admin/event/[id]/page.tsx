...
import { fmtDate, fmtMoney, fmtTime, isPast } from "@/lib/format";
...
export default async function EventBoard({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ copied?: string }> }) {
  ...
  const [{ data: pool }, { data: onDate }, { data: history }, { data: categories }, { data: siblings }, previous] = await Promise.all([
    // everyone ticked for this market, approved and not deleted
    supabase.from("stallholder_markets")
      .select("stallholders!inner(id, business_name, contact_name, status, category_id)")
      .eq("market_id", ev.market_id).eq("stallholders.status", "approved").is("stallholders.deleted_at", null),
    ...
    previousDates(supabase, org.id, id),
  ]);
  ...
  return (
    <>
      ...
      <div className="mt-2 flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-4xl font-bold">{fmtDate(ev.date, { weekday: "long", day: "numeric", month: "long" })}{ev.theme ? <span className="text-muted">, {ev.theme}</span> : null}</h1>
          <p className="mt-1 text-muted">{ev.market_name} · {fmtTime(ev.start_time)} to {fmtTime(ev.end_time)} · {ev.max_pitches} pitches · {fmtMoney(ev.fee_pence)} each{ev.note ? ` · ${ev.note}` : ""}{past ? " · Finished" : ""}</p>
        </div>
        <Link href={`/admin/event/${id}/edit`} className="btn btn-ghost">Edit date</Link>
      </div>
      <Board
        eventId={id}
        ...
        previous={previous[ev.market_id] ?? []}
        copiedOnCreate={copied ? Number(copied) : 0}
      />
    </>
  );
}
